...
class MSEYTrue:
    ...
    def make_signal_for_prior(self, time_length, uncertainty=0.5):
        ...
        true_signal_list = np.zeros(time_length)
        ...
        return input_signals, mu_p, sigma_p, true_signal_list

    def calc_mse(self):
        input_signal, _, _, true_signal_list = self.make_signal_for_prior(
            time_length=120,
            uncertainty=0.5,
        )
        ...
        if self.cfg['MODEL']['RESERVOIR'] == 0:
            _, output_list = self.model(inputs, hidden, 120)
        else:
            _, output_list, _ = self.model(inputs, hidden, reservoir, 120)

        mse = mean_squared_error(
            output_list[0, 20:, 0].detach().numpy(), 
            true_signal_list[20:],
        )

        return mse

    def evaluate_accuracy(self, sample_num=1000):
        mse = 0
...
if __name__ == '__main__':
    ...
    mse = mse_y_true.evaluate_accuracy(sample_num=args.sample_num)
